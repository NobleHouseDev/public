...
public final class SideEffectChecker extends BugChecker
    ...
    // These are ALWAYS impure. Things like network calls, file system read/write, random numbers, time.now, date.now
    ...
    private static final Matcher<ExpressionTree> ALL_KNOWN_THIRD_PARTY_CONTEXT_DEPENDENT_IMPURE_METHODS
            = anyOf(
                    JAVA_UTIL_LIST_ADD
            );
    ...
    public Description matchMethod(MethodTree tree, VisitorState state) {
        ...
        boolean isMethodMarkedAsPure = ASTHelpers.hasAnnotation(tree, "dev.noblehouse.side_effect.annotations.SideEffectIgnore", state);
        boolean isMethodMarkedAsSideEffect = ASTHelpers.hasAnnotation(tree, "dev.noblehouse.side_effect.annotations.SideEffect", state);

        // If the method is already annotated with @SideEffect, then we can exit early. (Future idea, If this plugin becomes very mature, then we should still process. If we don't find a side effect, then we should emit a warning saying the method is erroneously annotated.)
    ...
    private List<String> evaluateExpressionTree(ExpressionTree expressionTree, VisitorState state, Set<String> localVars) {
        ...
        if (expressionTree instanceof MethodInvocationTree methodInvocationTree) {
            ...
            if (ASTHelpers.hasAnnotation(ASTHelpers.getSymbol(methodInvocationTree), "dev.noblehouse.side_effect.annotations.SideEffect", state)) {
                ...
                errors.add("Should be annotated because it calls a impure function");
            } else if (!localVars.contains(variable) && ALL_KNOWN_THIRD_PARTY_CONTEXT_DEPENDENT_IMPURE_METHODS.matches(methodInvocationTree, state)) {
                errors.add("Should be annotated because it calls a impure function");
            }
        }
